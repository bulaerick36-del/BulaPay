...
// Purga automática de Service Workers y comunicados obsoletos en caché local (bulapay-v344)
window.forcePurgeAndRegisterServiceWorker = async function() {
  ...
  try {
    ...
    console.log('🧹 [PWA Purga bulapay-v344] Llaves de comunicados locales eliminadas para forzar tiempo real Supabase.');
  ...
  try {
    ...
    // 3. Registrar el nuevo Service Worker con parámetro de versión dinámico (v344)
    const swUrl = './sw.js?v=344&t=' + Date.now();
    const newReg = await navigator.serviceWorker.register(swUrl);
    await newReg.update();
    console.log('✔ Service Worker bulapay-v344 registrado con éxito (Fresh Register). Scope:', newReg.scope);

    if (window.bulaMobileDebugLog) {
      window.bulaMobileDebugLog('¡SW bulapay-v344 Registrado y Purgado con Éxito!', 'success');
    }

    const pwaStatus = document.getElementById('pwa-status');
    if (pwaStatus) pwaStatus.textContent = 'PWA Activa (bulapay-v344)';
  } catch (err) {
...
// Registro de Service Worker PWA con Auto-Destrucción y Re-registro Forzoso (bulapay-v344)
if ('serviceWorker' in navigator) {
  const triggerPurge = () => {
