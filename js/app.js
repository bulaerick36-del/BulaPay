...
window.forcePurgeAndRegisterServiceWorker = async function() {
  ...
  try {
    ...
    // 3. Registrar el nuevo Service Worker con parámetro de versión dinámico (v353)
    const swUrl = './sw.js?v=353&t=' + Date.now();
    const newReg = await navigator.serviceWorker.register(swUrl);
    await newReg.update();
    console.log('✔ Service Worker bulapay-v353 registrado con éxito (Fresh Register). Scope:', newReg.scope);

    if (window.bulaMobileDebugLog) {
      window.bulaMobileDebugLog('¡SW bulapay-v353 Registrado y Purgado con Éxito!', 'success');
    }

    const pwaStatus = document.getElementById('pwa-status');
    if (pwaStatus) pwaStatus.textContent = 'PWA Activa (bulapay-v353)';
  } catch (err) {
...
// Registro de Service Worker PWA con Auto-Destrucción y Re-registro Forzoso (bulapay-v353)
if ('serviceWorker' in navigator) {
  const triggerPurge = () => {
