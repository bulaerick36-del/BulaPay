...
// Purga automática de Service Workers y anuncios/notificaciones demo obsoletos (bulapay-v342)
window.forcePurgeAndRegisterServiceWorker = async function() {
  ...
  try {
    ...
    if (rawNotifs) {
      ...
      if (Array.isArray(parsedNotifs)) {
        const cleanedNotifs = parsedNotifs.filter(n => n && n.id && n.id !== 'notif_welcome');
  ...
  try {
    ...
    // 3. Registrar el nuevo Service Worker con parámetro de versión dinámico (v342)
    const swUrl = './sw.js?v=342&t=' + Date.now();
    const newReg = await navigator.serviceWorker.register(swUrl);
    await newReg.update();
    console.log('✔ Service Worker bulapay-v342 registrado con éxito (Fresh Register). Scope:', newReg.scope);

    if (window.bulaMobileDebugLog) {
      window.bulaMobileDebugLog('¡SW bulapay-v342 Registrado y Purgado con Éxito!', 'success');
    }

    const pwaStatus = document.getElementById('pwa-status');
    if (pwaStatus) pwaStatus.textContent = 'PWA Activa (bulapay-v342)';
  } catch (err) {
...
// Registro de Service Worker PWA con Auto-Destrucción y Re-registro Forzoso (bulapay-v342)
if ('serviceWorker' in navigator) {
  const triggerPurge = () => {
