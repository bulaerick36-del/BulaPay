...
const supervisorModule = {
  ...
  async init() {
    ...
    this.activeClientTab = 'supervisores';
    this.cachedClientsData = null;
    this.switchMainTab('routes');
  ...
  activeClientTab: 'supervisores',
  cachedClientsData: null,

  switchMainTab(tab) {
    const routesPanel = document.getElementById('panel-sup-routes');
    const clientsPanel = document.getElementById('panel-sup-clients');
    const btnRoutes = document.getElementById('btn-sup-tab-routes');
    const btnClients = document.getElementById('btn-sup-tab-clients');

    if (!routesPanel || !clientsPanel) return;

    if (tab === 'routes') {
      routesPanel.style.display = 'block';
      clientsPanel.style.display = 'none';
      if (btnRoutes) {
        btnRoutes.className = 'btn btn-primary';
      }
      if (btnClients) {
        btnClients.className = 'btn btn-secondary';
      }
    } else if (tab === 'clients') {
      routesPanel.style.display = 'none';
      clientsPanel.style.display = 'block';
      if (btnRoutes) {
        btnRoutes.className = 'btn btn-secondary';
      }
      if (btnClients) {
        btnClients.className = 'btn btn-primary';
      }
      this.loadAndRenderClients();
    }
  },

  switchClientTab(category) {
    this.activeClientTab = category;

    // Ocultar todos los contenidos de pestaña de clientes
    const tabContents = document.querySelectorAll('.client-tab-content');
    tabContents.forEach(el => el.style.display = 'none');

    // Mostrar el contenedor seleccionado
    const activeContent = document.getElementById(`client-tab-content-${category}`);
    if (activeContent) activeContent.style.display = 'block';

    // Actualizar estados visuales de las pestañas
    const tabs = ['supervisores', 'agentes-ruta', 'independientes', 'comercios'];
    tabs.forEach(t => {
    ...
    this.renderClientCategoryTable(category);
  },

  async loadAndRenderClients() {
    // Inyectar estado de cargando
    const categories = ['supervisores', 'agentes-ruta', 'independientes', 'comercios'];
    categories.forEach(cat => {
      const tbody = document.getElementById(`tbody-clients-${cat}`);
      if (tbody) {
        tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 1.5rem; color: var(--text-secondary);">Cargando información...</td></tr>`;
      }
    });
    // Pestaña de agentes de ruta tiene 4 columnas
    const tbodyRoute = document.getElementById('tbody-clients-agentes-ruta');
    if (tbodyRoute) {
      tbodyRoute.innerHTML = `<tr><td colspan="4" style="text-align: center; padding: 1.5rem; color: var(--text-secondary);">Cargando información...</td></tr>`;
    }

    try {
      const allClients = await window.BulaPayDB.getClients() || [];
      const allUsers = await window.BulaPayDB.getUsers() || [];
      const allRoutes = await window.BulaPayDB.getRoutes() || [];

      // Consulta relacional completa de clientes para Agentes de Ruta
      let routeAgentsClients = [];
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(`
            *,
            agent:users!agent_id ( username, name, role ),
            route:routes!routeId ( id, name, supervisor_id )
          `);
        if (error) throw error;
        
        // Filtrar solo los que correspondan a 'Agente de Ruta' o 'agent' en el frontend
        routeAgentsClients = (data || []).filter(c => c.agent && (c.agent.role === 'Agente de Ruta' || c.agent.role === 'agent'));
      } catch (err) {
        console.warn("Error en la consulta relacional de Supabase para agentes de ruta, usando fallback en memoria:", err);
        // Fallback en memoria
        routeAgentsClients = allClients
          .map(c => {
            const agent = allUsers.find(u => u.username === c.agent_id) || null;
            const route = allRoutes.find(r => r.id === c.routeId) || null;
            return {
              ...c,
              agent: agent ? { username: agent.username, name: agent.name, role: agent.role } : null,
              route: route ? { id: route.id, name: route.name, supervisor_id: route.supervisor_id } : null
            };
          })
          .filter(c => c.agent && (c.agent.role === 'Agente de Ruta' || c.agent.role === 'agent'));
      }

      // Clasificar los demás
      const supervisoresClients = [];
      const independientesClients = [];
      const comerciosClients = [];

      allClients.forEach(c => {
        let creator = allUsers.find(u => u.username === c.agent_id);
        if (!creator && c.supervisor_id) {
          creator = allUsers.find(u => u.username === c.supervisor_id);
        }

        if (creator) {
          const role = creator.role;
          if (role === 'Agente de Ruta' || role === 'agent') {
            // Ya manejado por la consulta relacional
          } else if (role === 'Agente Independiente' || role === 'independent') {
            independientesClients.push(c);
          } else if (role === 'Otros (Comercios, Compraventas, Mercados)' || role === 'Comercio Independiente' || role === 'commerce') {
            comerciosClients.push(c);
          } else if (role === 'Usuario Supervisor' || role === 'supervisor' || role === 'Administrador de Rutas') {
            supervisoresClients.push(c);
          } else {
            comerciosClients.push(c); // fallback
          }
        } else {
          if (c.routeId) {
            supervisoresClients.push(c);
      ...
      this.cachedClientsData = {
        supervisores: supervisoresClients,
        'agentes-ruta': routeAgentsClients,
        independientes: independientesClients,
        comercios: comerciosClients
      };

      // Renderizar la pestaña activa actual
      this.renderClientCategoryTable(this.activeClientTab);
    } catch (e) {
      console.error("Error al cargar y procesar clientes:", e);
      categories.forEach(cat => {
        const tbody = document.getElementById(`tbody-clients-${cat}`);
        if (tbody) {
          tbody.innerHTML = `<tr><td colspan="5" style="text-align: center; padding: 1.5rem; color: var(--color-rojo);">❌ Error al cargar los clientes.</td></tr>`;
        }
      });
    }
  },

  async renderClientCategoryTable(category) {
    if (!this.cachedClientsData) return;

    const clients = this.cachedClientsData[category] || [];
    const tbody = document.getElementById(`tbody-clients-${category}`);
    if (!tbody) return;

    tbody.innerHTML = '';

    if (clients.length === 0) {
      const colSpan = category === 'agentes-ruta' ? 4 : 5;
      tbody.innerHTML = `<tr><td colspan="${colSpan}" style="text-align: center; padding: 1.5rem; color: var(--text-muted);">No hay clientes registrados en esta categoría.</td></tr>`;
      return;
    }

    if (category === 'agentes-ruta') {
      const allUsers = await window.BulaPayDB.getUsers() || [];
      clients.forEach(c => {
        const supervisorUser = allUsers.find(u => u.username === c.route?.supervisor_id);
        const supervisorName = supervisorUser ? supervisorUser.name : 'Sin asignar';
        const agentName = c.agent ? c.agent.name : 'Sin asignar';
        const routeName = c.route ? c.route.name : 'Sin asignar';
      ...
      clients.forEach(c => {
        ...
        tr.innerHTML = `
          ...
          <td>${c.phone}</td>
          <td>${c.city} / ${c.zone}</td>
          <td style="color: var(--accent); font-weight: bold;">$${Number(c.outstanding).toLocaleString('es-CO')}</td>
        `;
        tbody.appendChild(tr);
      });
    }
